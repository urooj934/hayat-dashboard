      ...
      const sendmessage =(i,title)=>{
        try{
            firestore.collection('users').doc(i).collection('updates').add({
                user:i,
                title:'Object Donation',
                message :'Your donation of '+title+' has been verified ',
                createdAt:new Date() ,
            })}
            catch(error){
    ...
    <Section>
        ...
        <TableContainer component={Paper}>
            <Table sx={{border:3}} >
                <TableHead >
                  <TableRow>
                  <TableCell sx={{backgroundColor:'purple',color:'white',fontSize:'1.3rem',fontWeight:'bold'}} align="center" colSpan={11}>Objects Donations</TableCell>
                  </TableRow>
                    <TableRow>
                ...
                <TableBody>
                    ...
                    <TableCell>
                      ...
                      <Button onClick={()=>{handleverify(row.ID,row.uid,row.title)}} sx={{ color:'green',alignItems:'centre'}}><CheckCircleIcon/></Button>
                      </TableCell> 
                        </TableRow>
