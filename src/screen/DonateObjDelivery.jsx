...
const DonateObjDelivery=()=>{
      ...
      const sendmessage =(i,title)=>{
        try{
            firestore.collection('users').doc(i).collection('updates').add({
                user:i,
                title:'Object Donation Delivery',
                message :'Your donation of '+title+' has been delivered ',
    ...
    const handledelete=(ID)=>{
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
                firestore.collection('donateobject').doc(ID).delete().then(()=>
                {console.log('deleted');
            setDel(true);});
 ...
 //  if(DOC==="verified"){
  ...
  // }else{
      swal({
        title: "Are you sure?",
        text: "Mark this record delivered!",
        icon: "info",
        buttons: true,
      ...
      .then((willverify) => {
        ...
        setVerify(true);});
          swal("Record has been marked delivered!", {
            icon: "success",
          });
 ...
 //})
    ...
    const getDonateObjects =async()=>{
        const list =[];
        await  firestore.collection('donateobject').orderBy('createdAt','desc').get()
                  .then((querySnapshot)=>{
                      querySnapshot.forEach(doc=> {
                    const {uid,name,category,title,desc,phoneno,location,detailedaddress,status,createdAt,delivery}=doc.data();
                list.push({
                    ID:doc.id,
                    uid,
                    name,
    ...
    <Section>
        ...
        <TableContainer component={Paper}>
            <Table sx={{border:3}} >
                <TableHead >
                  <TableRow>
                  <TableCell sx={{backgroundColor:'purple',color:'white',fontSize:'1.3rem',alignSelf:'center',alignContent:'center',fontWeight:'bold'}}colSpan={12} align="center">Donated Objects Delivery</TableCell>
                  </TableRow>
                    <TableRow>
                    ...
                    <TableCell sx={{backgroundColor:'purple',color:'white',fontSize:'1.3rem',fontWeight:'bold'}}>Delivery</TableCell>
                ...
                <TableBody>
                    ...
                    <TableCell sx={{borderBottom:0.5,borderRight:0.5,borderColor:'#a19d9d'}}>{row.delivery}</TableCell>
                    <TableCell>
...
background-image: linear-gradient(-315deg, #21d190 0%, blueviolet 74%);
  ...
  `;
export default DonateObjDelivery;